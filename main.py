...
COLORS = {
    ...
    "text_dim": "#8080aa",
    "border": "#1a1a3a",
    # Couleurs supplémentaires pour le log coloré
    "orange": "#ff8800",
    "lime": "#aaff00",
    "pink": "#ff69b4",
    "gold": "#ffd700",
...
# Tags de couleur pour le log textbox (nom_tag, couleur_fg)
LOG_TAGS = {
    "header":   COLORS["cyan"],
...
def classify_log_line(line: str) -> str:
    ...
    if any(x in l for x in ["╔", "╚", "║"]):
        return "header"
    if "XOR" in l or "xor" in l.lower():
        return "xor"
    if any(x in l for x in ["S1:", "S2:", "S3:", "S4:", "S5:", "S6:", "S7:", "S8:", "S-Box", "S_Box"]):
        return "sbox"
    if any(x in l for x in ["Permutation", "Expansion", "PC-1", "PC-2", "IP", "IP⁻¹"]):
        return "perm"
    if any(x in l for x in ["K01", "K02", "K03", "K04", "K05", "K06", "K07", "K08",
                              "K09", "K10", "K11", "K12", "K13", "K14", "K15", "K16",
                              "sous-clé", "Clé", "Key Schedule", "rotate"]):
        return "key"
    if any(x in l for x in ["Round", "round", "─── Round"]):
        return "round"
    if any(x in l for x in ["Bloc #", "BLOC #"]):
        return "block"
    if any(x in l for x in ["✅", "Résultat final", "terminé", "chiffré", "COMPLETED"]):
        return "result"
    if any(x in l for x in ["┌", "└", "├", "│", "─"]):
        return "box"
...
class ResultCyberWindow:
    ...
    def setup_ui(self):
        ...
        self.success_icon = ctk.CTkLabel(header_frame, text="🎉", font=ctk.CTkFont(size=60))
        self.success_icon.pack()
        self.animate_icon()

        badge = ctk.CTkLabel(header_frame, text="⚡ SUCCÈS ⚡",
                             font=ctk.CTkFont(size=20, weight="bold"), text_color=COLORS["cyan"])
        badge.pack(pady=(5, 0))

        operation_text = "CHIFFREMENT" if self.operation == "encrypt" else "DÉCHIFFREMENT"
        title = ctk.CTkLabel(header_frame, text=f"{operation_text} COMPLET",
                             font=ctk.CTkFont(size=28, weight="bold"), text_color=COLORS["magenta"])
        title.pack()

        algo_label = ctk.CTkLabel(header_frame, text=f"┌── {self.algorithm_name} ──┐",
                                  font=ctk.CTkFont(size=12, family="Consolas"), text_color=COLORS["text_dim"])
        algo_label.pack()

        result_frame = ctk.CTkFrame(main_frame, corner_radius=15, fg_color=COLORS["bg_darker"],
                                    border_width=2, border_color=COLORS["purple"])
        result_frame.pack(fill="both", expand=True, padx=25, pady=20)

        result_title = ctk.CTkLabel(result_frame, text="░▒▓█ RÉSULTAT FINAL █▓▒░",
                                    font=ctk.CTkFont(size=12, family="Consolas", weight="bold"),
                                    text_color=COLORS["green"])
        result_title.pack(pady=(15, 5))

        self.result_text = ctk.CTkTextbox(result_frame, font=ctk.CTkFont(size=13, family="Consolas"),
                                          corner_radius=8, wrap="word", fg_color=COLORS["bg_card"], height=150)
        ...
        copy_btn = ctk.CTkButton(btn_frame, text="[ 📋 COPIER ]",
                                 command=lambda: self.copy_result(self.result),
                                 fg_color="transparent", border_width=2, border_color=COLORS["cyan"],
                                 hover_color=COLORS["cyan_dark"], width=160, height=45,
                                 font=ctk.CTkFont(size=14, weight="bold"), corner_radius=10)
        copy_btn.pack(side="left", padx=15)

        close_btn = ctk.CTkButton(btn_frame, text="[ FERMER ]", command=self.window.destroy,
                                  fg_color="transparent", border_width=2, border_color=COLORS["magenta"],
                                  hover_color=COLORS["magenta_dark"], width=140, height=45,
                                  font=ctk.CTkFont(size=14, weight="bold"), corner_radius=10)
...
class ProgressCyberWindow:
    """Fenêtre de traitement avec affichage coloré des étapes (XOR, S-Boxes, rounds...)"""
    ...
    def __init__(self, parent, algorithm, operation, text, key, update_result_callback):
        ...
        self.window = ctk.CTkToplevel(parent)
        self.window.title(f"[ {operation.upper()} PROCESS — DÉTAIL COMPLET ]")
    ...
    def setup_ui(self):
        main_frame = ctk.CTkFrame(self.window, corner_radius=20, fg_color=COLORS["bg_card"],
                                  border_width=2, border_color=COLORS["cyan"])
        ...
        # ── En-tête ──
        header_frame = ctk.CTkFrame(main_frame, fg_color=COLORS["bg_darker"],
                                    corner_radius=12, border_width=1, border_color=COLORS["purple"])
        header_frame.grid(row=0, column=0, sticky="ew", padx=20, pady=(18, 8))
        ...
        ctk.CTkLabel(header_frame,
                     ...
                     text_color=op_color).pack(anchor="w", padx=15, pady=(10, 3))

        text_preview = self.text[:60] + "..." if len(self.text) > 60 else self.text
        ctk.CTkLabel(header_frame,
                     text=f"  INPUT : {text_preview}",
                     font=ctk.CTkFont(size=11, family="Consolas"),
                     text_color=COLORS["text_dim"]).pack(anchor="w", padx=15, pady=2)
        ctk.CTkLabel(header_frame,
                     text=f"  KEY   : {self.key}",
                     font=ctk.CTkFont(size=11, family="Consolas"),
                     text_color=COLORS["text_dim"]).pack(anchor="w", padx=15, pady=(2, 10))

        # ── Barre de progression ──
        prog_frame = ctk.CTkFrame(main_frame, fg_color="transparent")
        prog_frame.grid(row=1, column=0, sticky="ew", padx=20, pady=(0, 6))

        self.progress_bar = ctk.CTkProgressBar(prog_frame, height=12, corner_radius=6,
                                               fg_color=COLORS["bg_darker"],
                                               progress_color=COLORS["cyan"])
        self.progress_bar.pack(fill="x")
        self.progress_bar.set(0)

        self.progress_label = ctk.CTkLabel(prog_frame, text="[>] INITIALIZING...",
                                           font=ctk.CTkFont(size=11, family="Consolas"),
                                           text_color=COLORS["cyan"])
        self.progress_label.pack(anchor="w", pady=(4, 0))

        # ── Légende des couleurs ──
        legend_frame = ctk.CTkFrame(main_frame, fg_color=COLORS["bg_darker"],
                                    corner_radius=8, border_width=1, border_color=COLORS["border"])
        legend_frame.grid(row=2, column=0, sticky="new", padx=20, pady=(0, 4))
        ...
        for label, color in legend_items:
            ctk.CTkLabel(legend_inner, text=label,
                         font=ctk.CTkFont(size=9, family="Consolas", weight="bold"),
                         text_color=color).pack(side="left", padx=6)

        # ── Zone de log principale ──
        log_outer = ctk.CTkFrame(main_frame, corner_radius=12, fg_color=COLORS["bg_darker"],
        ...
        ctk.CTkLabel(log_title_row,
                     text="┌─[ DÉROULEMENT DÉTAILLÉ : XOR · ROUNDS · S-BOXES · PERMUTATIONS ]─",
                     font=ctk.CTkFont(size=12, weight="bold", family="Consolas"),
                     text_color=COLORS["purple"]).pack(side="left")

        # Bouton clear log
        ctk.CTkButton(log_title_row, text="[ CLEAR ]",
                      command=self._clear_log,
                      width=80, height=26,
                      fg_color="transparent", border_width=1,
                      border_color=COLORS["text_dim"],
                      hover_color=COLORS["card_hover"],
                      font=ctk.CTkFont(size=10, family="Consolas")).pack(side="right")

        # Textbox avec scrollbar native
        ...
        scrollbar_y = tk.Scrollbar(log_outer, orient="vertical", command=self.log_text.yview)
        scrollbar_x = tk.Scrollbar(log_outer, orient="horizontal", command=self.log_text.xview)
        ...
        self.log_text.pack(fill="both", expand=True, padx=(10, 0), pady=(0, 0))
        ...
        # Tags spéciaux
        self.log_text.tag_configure("xor",    foreground=COLORS["yellow"],  font=("Consolas", 11, "bold"))
        self.log_text.tag_configure("round",  foreground=COLORS["magenta"], font=("Consolas", 11, "bold"))
        self.log_text.tag_configure("result", foreground=COLORS["green"],   font=("Consolas", 11, "bold"))
        self.log_text.tag_configure("header", foreground=COLORS["cyan"],    font=("Consolas", 11, "bold"))
        self.log_text.tag_configure("sbox",   foreground=COLORS["orange"])
        self.log_text.tag_configure("key",    foreground=COLORS["sky"])

        # ── Pied de page ──
        footer_frame = ctk.CTkFrame(main_frame, fg_color="transparent")
        footer_frame.grid(row=4, column=0, sticky="ew", padx=20, pady=(0, 12))

        self.footer_label = ctk.CTkLabel(footer_frame, text="[>] SYSTEM: PROCESSING...",
                                         font=ctk.CTkFont(size=11, family="Consolas"),
                                         text_color=COLORS["text_dim"])
        self.footer_label.pack(side="left")

        self.line_count_label = ctk.CTkLabel(footer_frame, text="lignes: 0",
    ...
    def log(self, text: str):
        ...
        self._line_count += 1
        try:
            self.line_count_label.configure(text=f"lignes: {self._line_count}")
            self.window.update_idletasks()
        except Exception:
            pass

    def log_matrix(self, title: str, data, step_num=None, total=None):
        """Affiche une matrice ou un tableau d'octets avec titre et formatage."""
        if not hasattr(self, 'window') or not self.window.winfo_exists():
            return
        ...
        try:
            if isinstance(data, (bytes, bytearray)):
                # Afficher en hex groupé par 8
                hex_str = data.hex().upper()
                row_size = 16  # 16 nibbles = 8 octets par ligne
                ...
                if data and isinstance(data[0], list):
                    ...
                    # Liste 1D — afficher en ligne de 8
    ...
    def update_progress(self, current: int, total: int):
        if total > 0 and self.window.winfo_exists():
            ...
            self.progress_label.configure(
                text=f"[>] PROGRESS: {int(p * 100)}%  — étape {current}/{total}"
            )
            ...
            try:
                ...
                def log_step(step_text, step_num=None, total=None):
                    ...
                    if step_num is not None and total is not None:
                        ...
                        # Auto-progression douce
                        frac = min(step_count[0] / total_steps, 0.95)
                        self.progress_bar.set(frac)
                    self.log(step_text)

                def log_matrix_cb(title, matrix, step_num=None, total=None):
                    step_count[0] += 1
                    if step_num is not None and total is not None:
                        self.update_progress(step_num, total)
                ...
                if self.window.winfo_exists():
                    ...
                    self.progress_label.configure(text="[>] PROGRESS: 100% — TERMINÉ")
                    self.footer_label.configure(
                ...
                def show_result():
                    self.window.destroy()
                    ResultCyberWindow(parent_window, final_result, final_operation, final_algo_name)
                ...
                if self.window.winfo_exists():
                    ...
                    self.parent.after(2600, lambda m=err_msg: messagebox.showwarning(
                ...
                if self.window.winfo_exists():
                    ...
                    self.window.after(2500, self.window.destroy)
                    self.parent.after(2200, lambda m=str(e): messagebox.showerror("Erreur", m))
...
class CryptoApp:
    ...
    def setup_ui(self):
        # Sidebar
        self.sidebar = ctk.CTkFrame(self.window, width=350, corner_radius=0,
        ...
        self.logo = ctk.CTkLabel(header, text="⚡", font=ctk.CTkFont(size=60),
                                 text_color=COLORS["cyan"])
        self.logo.pack(pady=(5, 0))
        self.animate_logo()

        ctk.CTkLabel(header, text="┌─[CRYPTO SUITE]─┐",
                     font=ctk.CTkFont(size=18, weight="bold", family="Consolas"),
                     text_color=COLORS["cyan"]).pack()
        ctk.CTkLabel(header, text="└─[COMPLETE EDITION]─┘",
                     font=ctk.CTkFont(size=11, family="Consolas"),
                     text_color=COLORS["purple"]).pack()

        ctk.CTkFrame(self.sidebar, height=2, fg_color=COLORS["cyan"]).pack(fill="x", padx=20, pady=20)

        categories = [
            ("🔐 CLASSIQUE (TP1)", ["Caesar Cipher", "Vigenère Cipher", "Affine Cipher",
                                    "Playfair Cipher", "Hill Cipher", "One-Time Pad (OTP)"]),
            ("⚡ SYMÉTRIQUE (TP2)", ["RC4", "DES / 3DES", "AES (128/192/256)", "Twofish", "Serpent"]),
            ("🔑 ASYMÉTRIQUE (TP3)", ["RSA", "Diffie-Hellman (DH)", "ECC / ECDH", "ElGamal"]),
            ("📊 HACHAGE (TP4)", ["MD5", "SHA-256", "SHA-512"]),
            ("✍️ SIGNATURES (TP5)", ["RSA Signature", "ElGamal Signature", "DSA / ECDSA", "Ed25519"]),
        ]

        self.algo_frame = ctk.CTkScrollableFrame(self.sidebar, fg_color="transparent",
        ...
        for cat_name, algos in categories:
            ctk.CTkLabel(self.algo_frame, text=f"├─[ {cat_name} ]─┤",
                         font=ctk.CTkFont(size=11, weight="bold", family="Consolas"),
                         text_color=COLORS["magenta"]).pack(anchor="w", pady=(10, 5))
            for algo_name in algos:
                if algo_name in self.algorithms:
                    btn = ctk.CTkButton(
                        ...
                        font=ctk.CTkFont(size=11, family="Consolas"),
                        fg_color="transparent", hover_color=COLORS["card_hover"], anchor="w"
                    )
        ...
        self.status_label = ctk.CTkLabel(status_frame, text="◇ SYSTEM: READY ◇",
                                         font=ctk.CTkFont(size=12, weight="bold", family="Consolas"),
                                         text_color=COLORS["bg_darker"])
        self.status_label.pack(pady=15)

        # Main content
        self.main_frame = ctk.CTkFrame(self.window, corner_radius=25, fg_color=COLORS["bg_card"],
                                       border_width=2, border_color=COLORS["cyan"])
        self.main_frame.grid(row=0, column=1, sticky="nsew", padx=25, pady=25)
        self.main_frame.grid_columnconfigure(0, weight=1)
        self.main_frame.grid_rowconfigure(3, weight=1)

        info_frame = ctk.CTkFrame(self.main_frame, corner_radius=15, fg_color=COLORS["bg_darker"],
                                  border_width=1, border_color=COLORS["purple"])
        info_frame.grid(row=0, column=0, sticky="ew", pady=(0, 20))

        self.algo_title = ctk.CTkLabel(info_frame, text="[>] SELECT ALGORITHM [<]",
                                       font=ctk.CTkFont(size=26, weight="bold", family="Consolas"),
                                       text_color=COLORS["cyan"])
        self.algo_title.pack(pady=(20, 8), padx=25)

        self.algo_desc = ctk.CTkLabel(info_frame, text="CHOOSE A CIPHER FROM THE SIDEBAR",
                                      font=ctk.CTkFont(size=13, family="Consolas"),
                                      text_color=COLORS["text_dim"], wraplength=800)
        self.algo_desc.pack(pady=(0, 20), padx=25)

        input_card = ctk.CTkFrame(self.main_frame, corner_radius=15, fg_color=COLORS["bg_darker"],
                                  border_width=1, border_color=COLORS["purple"])
        input_card.grid(row=1, column=0, sticky="ew", pady=(0, 20))
        input_card.grid_columnconfigure(0, weight=1)

        ctk.CTkLabel(input_card, text="┌─[ INPUT DATA ]─┐",
                     font=ctk.CTkFont(size=14, weight="bold", family="Consolas"),
                     text_color=COLORS["cyan"]).grid(row=0, column=0, sticky="w", padx=20, pady=(15, 5))

        self.input_text = ctk.CTkTextbox(input_card, height=130,
        ...
        controls_card = ctk.CTkFrame(self.main_frame, corner_radius=15, fg_color=COLORS["bg_darker"],
                                     border_width=1, border_color=COLORS["purple"])
        controls_card.grid(row=2, column=0, sticky="ew", pady=(0, 20))
        controls_card.grid_columnconfigure(0, weight=1)

        ctk.CTkLabel(controls_card, text="┌─[ ENCRYPTION KEY ]─┐",
                     font=ctk.CTkFont(size=13, weight="bold", family="Consolas"),
                     text_color=COLORS["magenta"]).grid(row=0, column=0, sticky="w", padx=20, pady=(15, 5))

        self.key_entry = ctk.CTkEntry(controls_card, placeholder_text="ENTER KEY...",
                                      font=ctk.CTkFont(size=14, family="Consolas"),
        ...
        self.encrypt_btn = GlowButton(btn_frame, text="[ ENCRYPT ]", command=self.encrypt_text,
                                      fg_color=COLORS["cyan"], hover_color=COLORS["cyan_dark"],
                                      height=55, font=ctk.CTkFont(size=16, weight="bold", family="Consolas"),
                                      corner_radius=12)
        self.encrypt_btn.grid(row=0, column=0, padx=10, sticky="ew")

        self.decrypt_btn = GlowButton(btn_frame, text="[ DECRYPT ]", command=self.decrypt_text,
                                      fg_color=COLORS["magenta"], hover_color=COLORS["magenta_dark"],
                                      height=55, font=ctk.CTkFont(size=16, weight="bold", family="Consolas"),
                                      corner_radius=12)
        self.decrypt_btn.grid(row=0, column=1, padx=10, sticky="ew")

        result_card = ctk.CTkFrame(self.main_frame, corner_radius=15, fg_color=COLORS["bg_darker"],
                                   border_width=1, border_color=COLORS["purple"])
        result_card.grid(row=3, column=0, sticky="nsew")
        result_card.grid_columnconfigure(0, weight=1)
        result_card.grid_rowconfigure(1, weight=1)

        ctk.CTkLabel(result_card, text="┌─[ LAST RESULT ]─┐",
                     font=ctk.CTkFont(size=13, weight="bold", family="Consolas"),
                     text_color=COLORS["green"]).grid(row=0, column=0, sticky="w", padx=20, pady=(15, 5))

        self.result_text = ctk.CTkTextbox(result_card, font=ctk.CTkFont(size=13, family="Consolas"),
                                          corner_radius=10, fg_color=COLORS["bg_card"],
                                          border_width=1, border_color=COLORS["purple"])
        self.result_text.grid(row=1, column=0, sticky="nsew", padx=20, pady=(0, 10))

        ctk.CTkButton(result_card, text="[ COPY ]", command=self.copy_result,
            ...
            row=2, column=0, sticky="e", padx=20, pady=(0, 15)
        )
    ...
    def select_algorithm(self, algo_name):
        ...
        for name, btn in self.algo_buttons.items():
            btn.configure(fg_color=COLORS["cyan"] if name == algo_name else "transparent")
        ...
        self.status_label.configure(text=f"◇ SYSTEM: {algo_name[:25].upper()} SELECTED ◇")
    ...
    def encrypt_text(self):
        ...
        if not text:
            ...
            return
        if not key and self.current_algorithm.get_key_info()["type"] != "hash":
