...
class HomomorphicVote:
    ...
    def tally_votes(self):
        ...
        total_votes = [0] * len(self.candidates)
...
class SecureApp:
    ...
    def _create_election(self):
        ...
        candidates_text.insert("1.0", "Candidat A\nCandidat B\nCandidat C")
    ...
    def _show_vote_dialog(self):
        ...
        def on_vote():
            ...
            success, msg = self.vote_system.cast_vote(
                self.current_user.name + str(time.time()),
                selected.get()
            )
            self.vote_log.insert(tk.END, f"{'✅' if success else '❌'} {msg}\n")
            self.vote_log.see(tk.END)
            messagebox.showinfo("Vote", msg)
            dlg.destroy()
        
        tk.Button(dlg, text="Confirmer", bg=YELLOW, fg="black", command=on_vote).pack(pady=20)
    
    def _tally_votes(self):
        if not self.vote_system or not self.vote_system.encrypted_votes:
            messagebox.showinfo("Dépouillement", "Aucun vote")
            return
        
        results = self.vote_system.tally_votes()
        self.vote_log.insert(tk.END, f"\n📊 RÉSULTATS\n")
        for cand, count in results.items():
            self.vote_log.insert(tk.END, f"  {cand}: {count} voix\n")
        self.vote_log.insert(tk.END, f"Total: {sum(results.values())} votes\n\n")
    
    def _show_results(self):
        if not self.vote_system or not self.vote_system.results:
            messagebox.showinfo("Résultats", "Aucun résultat")
            return
